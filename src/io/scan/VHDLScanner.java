...
public class VHDLScanner {
    ...
    private VHDLToken lastReadToken;
    ...
    public VHDLToken next() throws Exception {
        ...
        while (true) {
            ...
            if (lexeme == null){
                ...
                lastReadToken = allowIncompleteTokens && complexTokenValue.length() > 0 ?
                        lastReadToken = new VHDLToken(complexTokenType, complexTokenValue.toString().trim()) : null;
            ...
            if (lexeme.getType() == LexemeType.SEMICOLON) {
                ...
                return lastReadToken = new VHDLToken(complexTokenType, complexTokenValue.toString().trim());
            }
    ...
    public VHDLToken getLastReadToken() {
        return lastReadToken;
    }

    /*todo: 2(or 3) temporal methods: mark and reset (close) */
//    public void mark(int readAheadLimit) throws IOException {
//        bReader.mark(readAheadLimit);
//    }
//
    ...
    public void appendLexemeAndManageSpace(StringBuffer destinationStrBuf, Lexeme lexeme) { //todo: maybe rename to manageSpace (see PSLScanner) 
        ...
        if (!(lexeme.getType() == LexemeType.OP_SUBTR || lexeme.getType() == LexemeType.COLON
            ...
            * a space and the penultimate one was a digit), then remove the space inside the quotes and add a trailing one */
            ...
            /* If appended single quote followed an IDENTIFIER, then remove the trailing space (CLOCK' in CLOCK'EVENT) */
//            if (lexeme.getType() == LexemeType.SINGLE_QUOTE) {
//                if (destinationStrBuf.charAt(lastCharIndex) == ' '
//                        && Character.isLetter(destinationStrBuf.charAt(lastCharIndex - 1))) {
//                    destinationStrBuf.deleteCharAt(lastCharIndex);
//                }
//            }
        }
