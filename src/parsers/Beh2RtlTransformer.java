...
public class Beh2RtlTransformer {
	...
	private static final Logger LOG = Logger.getLogger(Beh2RtlTransformer.class.getName());
...
//                nodeIndex += graphVariable.getGraph().getRootNode().getSize();
    ...
    private void createFsmGraph(ControlPartManager controlPartOrdering) throws Exception {
		LOG.fine("Start creating FSM graph...");
		...
		LOG.fine("Processing STATE graph DONE.");
		if (!hasStateVar) {
			LOG.info("No state variable found");
        ...
        for (AbstractVariable absVariable : behModel.getVars().values()) {
			if (absVariable instanceof GraphVariable) {
				GraphVariable graphVariable = (GraphVariable) absVariable;
				if (graphVariable.isState()) continue;
				LOG.fine("Processing graph #" + i + " ( " + (total - i) + " in queue ) =========== " + graphVariable.getBaseVariable());
				graphVariable.traverse(graphCreator);
				i++;
            }
        }
...
//        graphVariable.getGraph().getRootNode().indexate(0);
//        nodeIndex += graphVariable.getGraph().getRootNode().getSize();
        modelCollector.addVariable(graphVariable);
		LOG.fine("FSM graph created successfully!");
    }
    ...
    public class ControlPartManager {
        ...
        public void insertTransition(Transitions transitions, Node terminalNode, GraphVariable graphVariable, boolean isResetingTerminal) throws Exception {
            AbstractVariable controlPartVariable;

            if (isResetingTerminal) {
