...
public class ExpressionBuilder {
    private static final Pattern STD_LOGIC_VECTOR_CONV = Pattern.compile(
            "^" + RegexFactory.createStringRegex("STD_LOGIC_VECTOR") + " \\( .+ \\)$"
    );
    private static final Pattern UNSIGNED_CONV = Pattern.compile(
            "^" + RegexFactory.createStringRegex("UNSIGNED") + " \\( .+ \\)$"
    );
    private static final Pattern CONV_INTEGER_CONV = Pattern.compile(
            "^" + RegexFactory.createStringRegex("CONV_INTEGER") + " \\( .+ \\)$"
    );
    private static final Pattern USER_DEFINED_CONV = Pattern.compile(
            "^[a-zA-Z][\\w]* \\( .+ \\)$"
    );

    public static final Pattern bitRangePattern  = Pattern.compile("^[^ ]+? \\( .+? (DOWNTO|TO) .+? \\)$");
    private static final Pattern singleBitPattern = Pattern.compile("^[^ ]+? \\( .+? \\)$"); // .+? \( .+? \)$
    private static final Pattern pureBitRangePattern = Pattern.compile("^.+? (DOWNTO|TO) .+$");
...
//        AbstractOperand abstractOperand = builder.buildExpression("( ( ( ROM_2 AND ( NOT IN_R ) ) OR ( ( ROM_1 AND IN_R ) OR ( ROM_2 AND ROM_1 ) ) ) =\"11111111\" )");
//        AbstractOperand abstractOperand = builder.buildExpression("VOTO0 ='1' AND VOTO1 ='1' AND VOTO2 ='1' AND VOTO3 ='1'");
//        AbstractOperand abstractOperand = builder.buildExpression("( ( VOTO0 ='1' ) AND ( ( VOTO1 ='1' ) AND ( ( VOTO2 ='1' ) AND ( VOTO3 ='1' ) ) ) )");
        AbstractOperand abstractOperand = builder.buildExpression("( ( voto0 = '0' ) AND ( ( voto1 = '1' ) AND ( ( voto2 = '1' ) AND ( voto3 = '0' ) ) ) )");
    ...
    public AbstractOperand buildExpression(String line) throws Exception {
        ...
        /* Trim enclosing brackets and whitespaces.
     ...
     * Regions of valid operator are those substrings that both represent the operator
     ...
     * for the operator ( with the hightest precedence).
     * Invalid regions are the substrings enclosed in brackets.
     *
    ...
    Region[] getValidRegions(String line) throws Exception {
        ...
        while (true) {
            ...
            /*
            * SubLine containts bracket. Add new ValidRegion and
...
//                else if (aChar == ')') {
//                    if (bracketCount == 0) return propertyBody; /* If this ')' closes previously trimmed '(', then the trim is not accepted! Undo trim.*/
//                    else bracketCount--;
//                }
//            }
//
...
//    }
    ...
    private static int getClosingIndex(String line, int startingCharIndex) throws Exception {
        ...
        switch (startChar) {
            ...
            case '{':
                ...
                break;
            default:
                throw new Exception("Cannot determine ending characted for the following UNKNOWN starting character: \'" + startChar + "\'");
        }
    ...
    public Expression unfoldBoolean(OperandImpl operand) throws Exception {
        Expression retExpr = new Expression(Operator.EQ, false);

        retExpr.addOperand(new OperandImpl(operand.getName(), operand.getPartedIndices(), false));
        retExpr.addOperand(operand.isInverted() ? buildExpression("\'0\'") : buildExpression("\'1\'"));
