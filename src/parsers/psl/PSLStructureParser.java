...
public class PSLStructureParser {
    ...
    private static final String PROPERTY_NAME_REGEX = "^" + LexemeType.IDENTIFIER.getRegex() + PROPERTY_DIRECTIVE_START + ".*";
...
//                    pslStructureBuilder.buildConfiguration(token);
    ...
    static String extractPropertyBody(String propertyLine) throws Exception {
        ...
        if (propertyBody.endsWith(PROPERTY_BODY_END)) { propertyBody = propertyBody.substring(0, propertyBody.length() - 1).trim(); }
    ...
    static void checkForPropertyName(String propertyLine) throws Exception {
        if (!propertyLine.matches(PROPERTY_NAME_REGEX)) throw new Exception(MISSING_NAME_TEXT + propertyLine);
    }
