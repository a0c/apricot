...
public class OperandLengthSetter {
                ...
                if (valAndLenHolder == null) {
                    ...
                    if (variable == null) {
                        if (variableName.equalsIgnoreCase("C_ADDR_OFFS_CRC_CFG")) {
                            ...
                            Object solution = ExceptionSolver.getInstance().findSolution(exception.getMessage(), ExceptionSolver.SolutionOptions.CONSTANT_VALUE);
                    ...
                    * Map operand, if its length is set (either by partedIndices or by own length) */
                    Indices length = null;
                    if (operand.isParted()) length = operand.getPartedIndices().deriveLength(); // length is derived: Some_operand<2:2> ==> the length is being set (0:0), not the real indices (2:2).
    ...
    /**
     * Tries to set Operator imposed length, if both it is not set yet and it is possible (deductable).
     * @param expression to check for constrainted length imposings on its operands
     */
    private void storeImposedLength(Expression expression) {
        ...
        if (isNonimposingOperator(operator)) {
            /* Do nothing, since CAT and SHIFTs don't impose any constrainted length. */
        } else {
