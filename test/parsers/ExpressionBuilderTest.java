...
public class ExpressionBuilderTest {
    ...
    private static final String[] trimSource = {
            "( something )",
            "( something () )",
            "( ()   ()  )",
            "()     ()",
    ...
    private static final String[] trimDest = {
            "something",
            "something ()",
            "()   ()",
            "()     ()",
    ...
    private static final String[] explicitTypeConversions = {
    ...
    private static final String[] nonExplicitTypeConversions = {
    ...
    private static final String[] clockExpressions = {
            "CLOCK'EVENT AND CLOCK='1'",
            "CLK='0'",
            "CLK'EVENT AND CLK='1'"
    };

    private static final String[] sourceIndices = {
            "NoIndices",
            "SingleIndex ( 92093 )",
    ...
    private static final Indices[] destIndices = {
            null,
    ...
    public void testTrimEnclosingBrackets() {
        ...
        for (int i = 0; i < trimSource.length; i++) {
            assertEquals(trimDest[i], ExpressionBuilder.trimEnclosingBrackets(trimSource[i]));
        }
    ...
    public void correctExpressionBuiltFromETC() throws Exception { // Explicit Type Conversion
        ...
        AbstractOperand expression = builder.buildExpression(explicitTypeConversions[0]);
        ...
        assertTrue(OperandImpl.class.getSimpleName() + " is built instead of " + Expression.class.getSimpleName() +
                " for expression: " + explicitTypeConversions[0], 
    ...
    public void correctExpressionBuiltForClock() throws Exception {
        ExpressionBuilder builder = new ExpressionBuilder();
        AbstractOperand abstractOperand = builder.buildExpression(clockExpressions[0]);
        assertTrue(OperandImpl.class.getSimpleName() + " is built instead of " + Expression.class.getSimpleName() +
                " for expression: " + clockExpressions[0], abstractOperand instanceof Expression);
        Expression clockExpression = (Expression) abstractOperand;
        assertTrue("AND operand was not created for expression: " + clockExpressions[0],
                clockExpression.getOperator() == Operator.AND);
        assertTrue("Incorrect number of operand for the following expression: " + clockExpressions[0],
    ...
    public void correctIndicesBuilt() throws Exception {
        ...
        for (int i = 0; i < sourceIndices.length; i++) {
            Indices indices = builder.buildIndices(sourceIndices[i]);
            assertEquals("Incorrect indices built from \"" + sourceIndices[i] + "\"",
                    destIndices[i], indices);
        }
    }
